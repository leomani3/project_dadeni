...
`RunManager` (persistent, InitScene) owns the deckbuilder run: it instantiates the player under itself so it survives scene loads, and passes combat input across scenes. `StartCombat(enemyPrefabs)` stores `CurrentEnemyGroup` + the player's and virtual camera's poses, deactivates every active MainScene root and **additively** loads `CombatScene`; `CombatManager.Start` makes its own scene active (so pooled combat spawns land in CombatScene and die with it), reads `RunManager.Player`/`CurrentEnemyGroup`, spawns the enemies on the grid and places the player. `CombatManager.EndCombat` → `RunManager.EndCombat` unloads `CombatScene`, reactivates MainScene's roots, restores both poses and only then calls `Player.OnCombatExit()` (the NavMeshAgent warp needs MainScene's `NavMeshSurface` re-enabled). MainScene state — the current room included — survives combat. CombatScene's `CameraRegister` still destroys MainScene's camera, which is why the virtual camera pose is restored by hand.

### Rooms and map
`MapData` is a placeholder linear map (a `RoomType` list) that also answers the topology questions: `GetNextRoomDirections(index)` and `TryGetNextRoomIndex(index, direction)`. A real branching map replaces it behind those two calls. `RunManager` maps `RoomType` → `Room` prefab (one `RoomBase` variant per type) and only ever has one room instantiated, in MainScene at the world origin, walking on MainScene's baked NavMesh. `SpawnRoom` destroys the current room, instantiates the next, enables the doors matching the next directions and warps the player to `Room.SpawnPoint`. A `Door` (`DoorDirection`, trigger collider + kinematic Rigidbody) locked until `Room.Complete()` calls `RunManager.EnterNextRoom(direction)` once the player stays inside it for `_enterDuration`, which fades to black and spawns the next room. `RunCanvas`'s complete-room button is a mock of the real completion condition.
